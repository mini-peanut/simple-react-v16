...
要想实现这个功能，我们需要**两颗更加完善的树**，在第一次渲染之后，React 最终得到一个 Fiber 树，它反映了用于渲染 UI 的应用程序的状态。这棵树通常被称为 **current 树（当前树）**。当 React 开始处理更新时，它会构建一个所谓的 **workInProgress 树（工作过程树）**，它反映了要刷新到屏幕的未来状态，并且所有工作都在 `workInProgress` 树的 Fiber 节点上执行。

刚刚说到的Fiber节点，作为一种数据结构，可以先看看它长什么样子，我挑选了一些目前我认为很有必要知道的一些属性罗列了下来
